...
Sea $f_{max}^\prime(w)$ la frecuencia máxima de $w$ entre las frecuencias de todas las provincias y sea $f_{min}^\prime(w)$ la frecuencia mínima distinta de $0$ sobre todas las provincias, luego la métrica generalizada \textit{$maxDif_g$(w)} se puede definir como en la ecuación \ref{eq:maxDifg}.
...
Una medida que se puede usar para comparar las frecuencias de las palabras en las diferentes regiones del país es la entropía definida por Shannon (ver el Apéndice \ref{sub:entropiaShannon}), debido a que nos brinda un valor que informe qué tan uniforme es la distribución de las frecuencias de cada palabra. La entropía es máxima cuando los eventos son equiprobables y mínima en el caso que la probabilidad de un evento es 1.
...
Dado un texto dividido en $P$ partes iguales llamadas \textit{ventanas}, se calcula la entropía  $H(w)$ sobre el vector de cantidad de ocurrencias en cada una de las $P$ ventanas.
Sea $(X_1,\dots,X_{23})$ un vector aleatorio con $X_i$ la cantidad de ocurrencias de la palabra $w$ en la provincia $i$ en un corpus con una determinada distribución de frecuencias para cada palabra y sea $X^\prime = H(X_1,\dots,X_{23})$. Luego se define $\widehat{H(w)} = \mathbb{E}(X^\prime)$, es decir la esperanza de $X$. Por lo tanto, $\widehat{H(w)}$ es el resultado de calcular la media de $H(w)$ entre todas las posibles permutaciones del texto.
%variable aleatoria $X$ la entropía de un texto aleatorio con una distribución de frecuencias por cada provincia determinada.
...
\begin{equation}
  \Delta I(s) = \sum_{w = 1}^{K} p(w) (\widehat{H(w)} - H(w))
\end{equation}

considerando un texto de $N$ palabras, con un vocabulario de $K$ palabras distintas, siendo $s = N/P$ y  $p(w)$ la frecuencia total de la palabra en el texto.
...
Finalmente, definen al valor de la información de una palabra como
 \begin{equation}
  \Delta I_w(w) = p(w) \,  (\widehat{H(w)} - H(w))  =  p(w) \, \Delta{H(w)}
 \end{equation}
...
Es por esto que realizamos una métrica similar que tenga en cuenta la diferencia de la entropía sobre la cantidad de personas que utilizan la palabra $w$. Agregándole el término $norm_u(w)$, que es la constante normalizadora de la cantidad de usuarios descripta en la ecuación \ref{eq:norm2} obtenemos el valor de la información de las personas $I_u$,

\begin{equation}
I_u(w) = norm_{u}(w) \cdot (\widehat{H}_{u}(w) - H_{u}(w))
\label{eq:iu}
\end{equation}
...
\end{equation}
\begin{equation}
 MIN_u = \min\limits_{p \in Palabras} cant_u(w)
\end{equation}
\begin{equation}
  MAX_u = \max\limits_{p \in Palabras} cant_u(w)
\end{equation}

y $cant_u(w)$ es el logaritmo sobre la cantidad de usuarios que utilizan dicha palabra en la Argentina, es decir $cant_u(w)= \log_2(cantidadUsuarios(w)))$.
...
\begin{figure}[!ht]\centering
  \begin{subfigure}[t]{0.49\textwidth}
   ...
   \end{subfigure}
   \caption{\subref{fig:cantNormUsuarios} Histograma de la normalización de la cantidad de ocurrencias de las palabras, definida como $norm_u$. \ref{fig:cantNormPalabras} Histograma de la normalización sobre la cantidad de usuarios que utilizan una determinada cantidad de palabras definida como $norm_w$.}
...
En el gráfico de la Figura \ref{fig:propAcum5000} se observa que la variación del cubrimiento de ocurrencias es menor a medida que se aumenta la cantidad de provincias. 
