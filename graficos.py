...
def histCreated(df):
    ax = df.groupby(level=0).get_group('lapampa').drop_duplicates(subset='user_id').created_at.hist(bins=100,alpha=0.5,color='red',label = 'La Pampa')
    df.groupby(level=0).get_group('buenosaires').drop_duplicates(subset='user_id').created_at.hist(bins=100,alpha=0.5,color='green',label = 'Buenos Aires',ax=ax)
    ax.legend()
...
def entropyVsNormCantPersonas(df):
    df.sort_values(by="information_value_personas",ascending=False,inplace=False)
...
def distribucionEntropia(df):
    ...
    ax.set_yscale('log')
    ax.get_figure().savefig("{0}DistribucionEntropia{1}{2}.pdf".format(relativePathImages,path,pathFiltro),dpi=300)
...
def distCantPalabra(df):
    ...
    ax.get_figure().savefig("{0}DistribucionOcurrenciasPalabras{1}{2}.pdf".format(relativePathImages,path,pathFiltro),dpi=300)
...
def rankIValue(df):
    ...
    ax.get_figure().savefig("{0}valorInformacionCorte{1}{2}.pdf".format(relativePathImages,path,pathFiltro),dpi=300)
...
def originalVsShuffle(df):
    ...
    ax.legend()
    ax.get_figure().savefig("{0}entropiaYversionShuffleada{1}{2}.pdf".format(relativePathImages,path,pathFiltro),dpi=300)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('notebooks/ivalue_entropia_personas_palabras{0}{1}.csv'.format(path,pathFiltro),index_col=0)
