# coding: utf-8
from textos import dictionary, cant_palabras
import enchant
import unicodecsv
import datetime
from apps import *
...
if __name__ == "__main__":
    ...
    df_resultado = df_resultado.rename(columns={'Unnamed: 0': 'Palabra'})
    ...
    dfLimpia = df_resultado[-df_resultado.index.isin(palabrasASacar)]
    print df_resultado.shape[0], dfLimpia.shape[0]

    # Le agrego la columna de MaxDif
    idmax = dfLimpia.filter(regex=("fnorm.*")).idxmax(1)
    maxi = dfLimpia.filter(regex=("fnorm.*")).max(1)
    dfLimpia['provFnormMax'] = idmax
    dfLimpia['FnormMax'] = maxi
    idmin = dfLimpia.filter(
        regex=("fnorm.*"))[dfLimpia > 0.0000000000000001].idxmin(1)
    mini = dfLimpia.filter(
        regex=("fnorm.*"))[dfLimpia > 0.0000000000000001].min(1)
    dfLimpia['provFnormMin'] = idmin
    dfLimpia['FnormMin'] = mini
    # print dfLimpia.columns
    minFnorm = dfLimpia['FnormMin'].min()
    print minFnorm
    dfLimpia['maxDif'] = dfLimpia.FnormMax / dfLimpia.FnormMin
    print dfLimpia[['provFnormMax', 'provFnormMin', 'FnormMax', 'FnormMin', 'maxDif']].head(20)

    dfLimpia.to_excel('dfLimpia.xls')
    agregarSugerencias(dfLimpia)
    dfLimpia.to_excel('palabrasClean.xls')
